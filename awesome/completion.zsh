...
_fzf_complete_sudo () {
  ...
  if [[ $args == 'sudo pacman -S'* ]]
  then
    matches="$(pacman -Slq | fzf -m --preview 'pacman -Si {}' --reverse --height ${FZF_TMUX_HEIGHT:-50%} $FZF_DEFAULT_OPTS $FZF_COMPLETION_OPTS | tr '\n' ' ')"
    ...
    zle reset-prompt
  elif [[ $args == 'sudo pacman -R'* ]]
  then
    matches="$(pacman -Qq | fzf -m --preview 'pacman -Qi {}' --reverse --height ${FZF_TMUX_HEIGHT:-50%} $FZF_DEFAULT_OPTS $FZF_COMPLETION_OPTS | tr '\n' ' ')"
