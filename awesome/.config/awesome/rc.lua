...
-- Autorun apps at start
awful.spawn.with_shell("~/.config/awesome/load.sh")
...
{description = "take a screenshot to a selection", group = "hotkeys"}),
    ...
    -- X screen locker  --scrlocker-- (slock)
    awful.key({ altkey, "Control" }, "l", function () os.execute("i3lock-fancy-dualmonitor -p") end,
...
awful.key({ "Control" }, "space", function () kbdcfg.switch() end),
 ...
 awful.key({ modkey }, "a", function () os.execute("sh ~/.config/awesome/load.sh") end,
    ...
    awful.key({ modkey }, "d", function ()
            os.execute(string.format("rofi -combi-modi drun -show combi -modi combi"))
        end,
...
{ rule_any = { class = { "Thunar", "Nautilus" } },
